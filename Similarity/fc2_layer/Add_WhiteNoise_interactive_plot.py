...
'''
For the following code, we want to add different scales of white noises (transformation)
to our motabolite image and then test their similarity levels. In the meantime, we process
the metabolite image through a transfer learning model before appling the same similarity test.

We want to test if the transfer learning model would do a better job at recognizing the motablite 
image even after transformations. In other words, we are interested to learn what transfer learning 
really do to our motabolite image. We use matrix norm of the image as a scaler for similarities.
'''

# ----------------------------------------------- data clean & transform -----------------------------------------------
def load_data(i_img):
    data_dir = '/directory/to/image/file'
    os.chdir(data_dir)
...
# ----------------------------------------------- transfer learning -----------------------------------------------
# resize images to fit the tranfer-learning model (VGG16: 1 channel gray-scale(60, 12, 1) to 3 channels RGB(224, 224, 3))
...
def choose_model(which_model):
    ...
    # stop transfer learning at 'fc2' layer (with 4096 parameters) before the final 'prediction' layer
...
# loop throgh different scale/percentage of transformation
